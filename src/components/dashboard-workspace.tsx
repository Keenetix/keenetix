...
export function DashboardWorkspace() {
  ...
  return <section className="dashboard-shell">
    ...
    {view === "keys" && <section className="api-key-view"><div className="api-key-intro"><p className="section-label">API credentials</p><h2>Keys for your<br /><em>agent runtime.</em></h2><p>Keys are scoped to this developer workspace. Store them in your secrets manager and never expose them in a client bundle.</p><button className="button button-coral" onClick={() => void createKey()} disabled={keyLoading}>{keyLoading ? "Creating key…" : "Create API key"} <ArrowIcon /></button></div><div className="api-key-panel">{newKey && <div className="new-key"><b>Copy this key now</b><code>{newKey}</code><button onClick={() => void navigator.clipboard.writeText(newKey)}>Copy</button><small>For security, it will not be shown again.</small></div>}<div className="key-table"><div className="key-table-head"><span>Name & scopes</span><span>Key</span><span>Created</span><span /></div>{data.apiKeys.length ? data.apiKeys.map((key) => <div key={key.id}><b>{key.name}<small>{key.scopes.join(" · ")}</small></b><code>{key.keyPrefix}</code><small>{formatDate(key.createdAt)}</small><button onClick={() => void revokeKey(key.id)}>Revoke</button></div>) : <p>No API keys created yet.</p>}</div></div></section>}
